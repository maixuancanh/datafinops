...
export function registerConnectionRoutes(app: FastifyInstance, store: ConnectionStore): void {
  app.post('/v1/connections', async (request, reply) => {
    const body = request.body as Partial<{
      ...
      readCredentialSecretRef: string;
    ...
    if (required.some((key) => !body[key]?.trim()))
      return reply.code(400).send({ error: 'connection fields are required' });
    const connection = store.create({
      tenantId: body.tenantId!,
      environmentId: body.environmentId!,
      secretRef: body.readCredentialSecretRef!,
      expectedNetwork: body.expectedNetwork!,
      expectedProgramId: body.expectedProgramId!,
      expectedApiHost: body.expectedApiHost!,
      publicWalletRef: body.publicWalletRef!,
    });
    return reply
      .code(201)
      .send({ connection, verification: { status: 'PENDING', executable: false } });
  });
}
