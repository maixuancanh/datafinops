...
describe('append-only canonical audit events', () => {
  it('creates a deterministic hash chain with tenant, actor, action, and purpose', () => {
    const first = createAuditEvent({
      ...
      actorRef: 'auditor-demo',
      action: 'PROPOSAL_READ',
      purpose: 'quarterly control review',
      resourceRef: 'proposal-demo',
    ...
    const second = recordPrivilegedRead({
      ...
      workspaceId: 'workspace',
      environmentId: 'environment',
      actorRef: 'support-demo',
      resourceRef: 'finance-export-demo',
      purpose: 'customer ticket DF-100',
