...
describe('TxLINE adapter contract', () => {
  ...
  it('blocks mismatch nondestructively and marks unknown semantics non-executable', async () => {
    const adapter = createReplayAdapter({
      mode: 'LIVE_READ',
      dataset: { ...dataset, apiHost: 'unknown' },
      clock: '2026-08-01T12:00:00.000Z',
    });
  ...
  it('records activation semantics but keeps source uncertainty explicit', async () => {
    ...
    const activation = await adapter.activateEntitlement({
      operationKey: 'activation-demo-0001',
      target: ['EPL'],
    });
