...
const dataset = {
  ...
  subscriptions: [],
  fixtures: [],
  entitlement: { status: 'UNKNOWN', leagues: [], latencyClass: 'UNKNOWN' },
...
describe('TxLINE capability modes', () => {
  ...
  it('requires explicit evidence before a live-write adapter can expose a write method', () => {
    expect(() =>
      createReplayAdapter({ mode: 'LIVE_WRITE', dataset, clock: '2026-08-01T12:00:00.000Z' }),
    ).toThrow(/evidence/i);
    const adapter = createReplayAdapter({
      mode: 'LIVE_WRITE',
      liveWriteEvidence: 'tenant-demo-evidence',
      dataset,
      clock: '2026-08-01T12:00:00.000Z',
    });
