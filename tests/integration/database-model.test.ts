...
const migrationPath = path.resolve(
  import.meta.dirname,
  '../../infrastructure/migrations/0001_baseline.sql',
);
...
const expectedTables = [
  ...
  'memberships',
  'service_accounts',
  'txline_connection_versions',
  'pricing_snapshots',
  'pricing_items',
  'subscription_versions',
  'entitlement_verifications',
  'fixture_schedule_versions',
  'products',
  'cost_centers',
  'usage_imports',
  'usage_aggregates',
...
describe('baseline database model', () => {
  ...
  it('enforces append-only approval, audit, outbox, and savings entries', async () => {
    const org = await client.query<{ id: string }>(
      "INSERT INTO organizations(name) VALUES('Demo') RETURNING id",
    );
    ...
    await expect(
      client.query('UPDATE approvals SET role=$1 WHERE id=$2', ['ADMIN', approval.rows[0]?.id]),
