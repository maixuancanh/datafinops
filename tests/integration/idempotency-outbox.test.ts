...
beforeAll(async () => {
  ...
  await pool.query("INSERT INTO workspaces(id,organization_id,name) VALUES($1,$2,'Demo')", [
    scope.workspaceId,
...
describe('idempotency and transactional outbox', () => {
  it('returns the original result and creates one outbox event on request replay', async () => {
    const operationKey = createOperationKey('usage-import', scope, {
      source: 'demo',
      period: '2026-08',
    });
    let executions = 0;
    ...
    const outbox = await pool.query('SELECT * FROM outbox_events WHERE operation_key=$1', [
      operationKey,
    ]);
  ...
  it('rolls back domain result and outbox together when the handler fails', async () => {
    ...
    expect(
      (
        await pool.query('SELECT 1 FROM idempotent_operations WHERE operation_key=$1', [
          operationKey,
        ])
      ).rowCount,
    ).toBe(0);
    expect(
      (await pool.query('SELECT 1 FROM outbox_events WHERE operation_key=$1', [operationKey]))
        .rowCount,
    ).toBe(0);
  });
  ...
  it('deduplicates reordered consumer delivery by consumer and operation key', async () => {
    ...
    const duplicate = await consumeOnce(
      pool,
      scope,
      'baseline-projector',
      operationKey,
